...
export function FileUploadInput({
  ...
  onChange,
  token,
  fileType = "image", // "image" | "video" | "any"
  aspectHint = "PNG, JPG, WEBP, MP4, WEBM format",
  multiple = false,
  onMultipleChange
}) {
          ...
          canvas.toBlob(
            (blob) => {
              ...
              const compressedFile = new File(
                [blob],
                (file.name || 'upload-file').replace(/\.[^/.]+$/, '') + '.jpg',
  ...
  const handleUploadFiles = async (files) => {
    ...
    try {
      ...
      setErrorMsg('Failed to process or upload file. Please try again.');
    } finally {
  ...
  const isVideoValue = value && (value.includes('.mp4') || value.includes('.webm') || value.includes('.mov') || value.startsWith('data:video/'));
      ...
      {value ? (
        <div className="relative group border border-outline-variant rounded overflow-hidden bg-surface-container-high p-2.5 flex items-center space-x-3 shadow-inner">
          {isVideoValue ? (
            <video
              ...
              className="w-20 h-16 object-cover rounded border border-outline-variant shrink-0 bg-black"
              muted
              loop
              autoPlay
              playsInline
          ...
          <div className="flex-1 min-w-0">
            <span className="text-[10px] text-primary font-bold uppercase tracking-widest block flex items-center space-x-1">
              ...
              <span>{isVideoValue ? 'Video File Uploaded' : 'Image File Uploaded'}</span>
            </span>
        ...
        >
          <input
            ref={fileInputRef}
            type="file"
            accept={fileType === 'video' ? 'video/mp4,video/webm,video/quicktime,video/*' : fileType === 'any' ? 'image/*,video/*' : 'image/*'}
            multiple={multiple}
            className="hidden"
          ...
          {isUploading ? (
            <div className="flex flex-col items-center justify-center space-y-1.5 text-primary py-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider">Uploading File...</span>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center space-y-1.5 text-on-surface-variant py-1">
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary mb-1">
                <span className="material-symbols-outlined text-xl">{fileType === 'video' ? 'movie' : 'cloud_upload'}</span>
              </div>
              <div className="text-xs font-semibold text-on-surface">
                <span className="text-primary underline font-bold">Choose {fileType === 'video' ? 'Video File' : 'Image/Media File'}</span> or Drag & Drop
              </div>
